...
import {
    Lock, Shield, Loader2, Save, UserCircle, AlertTriangle, X, Eye, EyeOff, Check, Mail, Phone, Menu
} from 'lucide-react';
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .update({
                    ...
                    // Keeping full_name update for backward compatibility if other tables rely on it
            ...
            toast.success("Doctor credentials locked successfully");
        ...
        <PageTransition>
            <div className="flex min-h-screen bg-[#F8FAFC]">
                ...
                <div className="flex-1 flex flex-col min-w-0 md:ml-20 lg:ml-72">
                    <main className="p-6 lg:p-12 space-y-12 overflow-y-auto">
                        ...
                        <header className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-slate-500 font-black uppercase text-[10px] tracking-[0.3em] mt-1">System Configuration & Identity</p>
                            </div>
                        </header>

                        <div className="max-w-4xl space-y-10">
                            {/* IDENTITY SECTION */}
                            <section className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-6 lg:p-10 shadow-sm space-y-8">
                                ...
                                {profile.name_updated && (
                                    <div className="bg-slate-50 p-5 rounded-2xl border-2 border-slate-100 flex flex-col md:flex-row items-start md:items-center gap-3">
                                        <AlertTriangle size={18} className="text-amber-500 shrink-0" />
                                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">
                                            Official medical details are locked for security compliance.
                                        </p>
                                    </div>
                                )}
                                ...
                                <form onSubmit={triggerWarning} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    {!profile.name_updated && (
                                        <div className="md:col-span-2 pt-4">
                                            <button type="submit" disabled={updating} className="flex w-full md:w-auto items-center justify-center gap-3 bg-black text-white px-10 py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-emerald-600 transition-all shadow-xl disabled:bg-slate-200">
                            ...
                            {/* SECURITY SECTION */}
                            <section className="bg-black rounded-[2.5rem] p-6 lg:p-10 text-white shadow-2xl space-y-8">
                                ...
                                <div onClick={() => setShowPasswordModal(true)} className="p-6 bg-white/5 rounded-3xl border border-white/10 flex items-center justify-between group hover:bg-white/10 transition-all cursor-pointer">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Update Security Credentials</p>
                                        </div>
                                    </div>
                                </div>
                            </section>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl animate-in zoom-in-95">
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-10 leading-relaxed">
                            This action will permanently lock your doctor identity. It can <span className="text-red-500 underline decoration-2">only be performed once</span> for system integrity.
                        </p>
            ...
            {showPasswordModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <form onSubmit={handleChangePassword} className="relative bg-white w-full max-w-md rounded-[3rem] p-8 lg:p-10 shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {requirements.map((req, index) => (
                                            <div key={index} className="flex items-center gap-2">
                                                <div className={`p-0.5 rounded-full shrink-0 ${req.met ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-300'}`}>
                                                    <Check size={8} strokeWidth={5} />
                                                </div>
                                                <span className={`text-[8px] font-black uppercase tracking-tight leading-tight ${req.met ? 'text-slate-900' : 'text-slate-400'}`}>{req.label}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
